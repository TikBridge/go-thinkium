...
func (bnh *BlockNumberOrHash) GetRealHeight(cdata models.DataHolder) common.Height {
	...
	} else if blockNr, ok := bnh.Number(); ok {
		if blockNr < 0 {
			// use current state for LatestBlockNumber/PendingBlockNumber
			return common.NilHeight
		}
		return common.Height(blockNr)
	} else {
		return cdata.GetCurrentHeight()
	}
}
...
func (bnh *BlockNumberOrHash) Number() (BlockNumber, bool) {
	if bnh.BlockNumber != nil || *bnh.BlockNumber == PendingBlockNumber || *bnh.BlockNumber == LatestBlockNumber {
...
func GenRpcTxRes(tx *models.Transaction, txI *models.TXIndex, recepit *models.Receipt) (*RPCTransaction, error) {
	...
	res := &RPCTransaction{
		...
		Type:             (hexutil.Uint64)(tx.ETHTxType()),
		Accesses:         &accl,
		ChainID:          (*hexutil.Big)(tx.ETHChainID()),
		V:                (*hexutil.Big)(extra.V),
		R:                (*hexutil.Big)(extra.R),
		S:                (*hexutil.Big)(extra.S),
	}
	return res, nil
}
