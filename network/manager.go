...
func (m *Manager) Status() {
	m.logger.Infof("--------------------CONNECT STATUS %s-------------------------", config.VersionInfo.VersionString())
	...
	for _, chainid := range chainids {
		...
		if nw, ok := value.(*NetWorker); ok {
			...
			if holder, err := m.dmanager.GetChainData(chainid); err == nil && holder != nil {
				currentHeight := holder.GetCurrentHeight()
				if holder.IsSynced() {
					syncing = fmt.Sprintf("SYNC(%s)", &currentHeight)
				} else {
					syncing = fmt.Sprintf("NOTSYNC(%s)", &currentHeight)
				}
				timedout = holder.TimedOut()
			}
			if timedout {
				m.logger.Errorf("ChainID:%d\tnetType:%v\tOP:%v\t%s\tTimeout",
...
func (m *Manager) InitNet(chaininfo *common.ChainInfos) error {
	if m.IsBootNode(chaininfo.ID) {
		...
		case common.Branch:
			...
			if err := m.CreateOrConnectNet(common.RootDataNet, chaininfo.ParentID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("create branch.root failed: %v", err)
			}
			...
			if ids != nil && ids.Len() > 0 && chaininfo.ParentID.IsMain() {
				if err := m.CreateOrConnectNet(common.BranchDataNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
					m.logger.Warnf("create branch.data failed: %v", err)
				}
			}
			if err := m.CreateOrConnectNet(common.BasicNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("create branch.basic failed: %v", err)
			}
		case common.Shard:
			if err := m.CreateOrConnectNet(common.RootDataNet, common.MainChainID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("create shard.root failed: %v", err)
			}
			if err := m.CreateOrConnectNet(common.BranchDataNet, chaininfo.ParentID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("create shard.data failed: %v", err)
			}
			if err := m.CreateOrConnectNet(common.BasicNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("create shard.basic failed: %v", err)
			}
		}
	}
	return nil
...
func (m *Manager) ConnectNet(chaininfo *common.ChainInfos) error {
	if err := m.CreateOrConnectNet(common.ConsensusNet1, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
		m.logger.Warnf("connect ConsNet1 failed: %v", err)
	}
	if err := m.CreateOrConnectNet(common.ConsensusNet2, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
		m.logger.Warnf("connect ConsNet2 failed: %v", err)
	}
	switch chaininfo.Mode {
	case common.Root:
		if err := m.CreateOrConnectNet(common.RootDataNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect root.root failed: %v", err)
		}
	case common.Branch:
		// branch chain join root data net
		if err := m.CreateOrConnectNet(common.RootDataNet, chaininfo.ParentID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect branch.root failed: %v", err)
		}
		// shard chain's parent should join branch data net
		ids := m.dmanager.GetChainChildren(chaininfo.ID)
		if ids != nil && ids.Len() > 0 && chaininfo.ParentID.IsMain() {
			if err := m.CreateOrConnectNet(common.BranchDataNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
				m.logger.Warnf("connect branch.data data failed: %v", err)
			}
		}
		if err := m.CreateOrConnectNet(common.BasicNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect branch.basic failed: %v", err)
		}
	case common.Shard:
		if err := m.CreateOrConnectNet(common.RootDataNet, common.MainChainID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect shard.root failed: %v", err)
		}
		if err := m.CreateOrConnectNet(common.BranchDataNet, chaininfo.ParentID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect shard.data failed: %v", err)
		}
		if err := m.CreateOrConnectNet(common.BasicNet, chaininfo.ID, chaininfo.ID); err != nil && config.IsLogOn(config.NetLog) {
			m.logger.Warnf("connect shard.basic failed: %v", err)
		}
	}
	return nil
}

func (m *Manager) ReviveNet(chaininfo *common.ChainInfos, epoch common.EpochNum) error {
	consensusNet := common.ChooseConNetByEpoch(epoch)
	err := m.CreateOrConnectNet(consensusNet, chaininfo.ID, chaininfo.ID)
	if err != nil {
		return err
	}
	if !chaininfo.IsMainChain() {
		err = m.CreateOrConnectNet(common.BasicNet, chaininfo.ID, chaininfo.ID)
	}
	return err
}
...
func (m *Manager) Initializer() error {
	if err := m.InitChain(common.MainChainID); err != nil {
		return err
	}
...
func (m *Manager) Starter() error {
	...
	if addr != nil {
		...
		if !common.StandAlone {
			if err := m.CreateOrConnectNet(common.RootDataNet, common.MainChainID, common.MainChainID); err != nil {
				return err
			}
			// create main chain consensus network
	...
	for k := range m.getAllNetInfomap() {
		...
		if err := m.InitChain(k); err != nil {
			return fmt.Errorf("failed at InitChain(ChainID:%d): %v", k, err)
		}
		// join net
		if err := m.InitNet(chaininfo); err != nil {
			return fmt.Errorf("failed at InitNet(%s): %v", chaininfo, err)
		}
	}

	if m.dmanager.IsMemoNode() {
		if err := m.InitChain(*common.ForChain); err != nil {
			return fmt.Errorf("failed at memo node InitChain(ChainID:%d): %v", *common.ForChain, err)
		}
	}
	...
	if m.dmanager.IsDataMemoNode() {
		...
		if ok {
			if err := m.InitChain(*common.ForChain); err != nil {
				return fmt.Errorf("failed at data memo node InitChain(ChainID:%d): %v", *common.ForChain, err)
			}
			if err := m.ConnectNet(chaininfo); err != nil {
				return fmt.Errorf("failed at data memo node ConnectNet(%s): %v", chaininfo, err)
			}
		}
	}
...
func (m *Manager) IsBootNode(id common.ChainID) bool {
	...
	_, exist := mm[common.SystemNodeID]
	return exist
}
...
func (m *Manager) CreateOrConnectNet(ntp common.NetType, bootChainID, localChandID common.ChainID) error {
	...
	if boots, ok := m.GetChainNet(bootChainID, ntp); ok {
		...
		if addr != nil {
			...
			if config.IsLogOn(config.NetLog) {
				m.logger.Infof("%s boot node created, ChainID:%d Addr:%s", ntp, bootChainID, addr)
			}
			...
			if config.IsLogOn(config.NetLog) {
				m.logger.Infof("connect to boot node, ChainID:%d -> ChainID:%d netType:%s",
			...
			if err := vt.Connect(ntp, bootChainID, boots, infos, nil, nil); err != nil {
				if err == ErrAlreadyConnected {
					if config.IsLogOn(config.NetLog) {
						m.logger.Warnf("connect to boot node, ChainID:%d -> ChainID:%d netType:%s warning %v",
							localChandID, bootChainID, ntp, err)
					}
				} else {
					if config.IsLogOn(config.NetLog) {
						m.logger.Errorf("connect to boot node, ChainID:%d -> ChainID:%d netType:%s error %v",
							localChandID, bootChainID, ntp, err)
					}
					return err
				}
...
func (m *Manager) ResetNet(chainid common.ChainID, ntp common.NetType) error {
	if m.IsBootNode(chainid) {
		if err := m.ResetOneNet(chainid, ntp); err != nil {
			return err
		}
	}
	return nil
