...
running:
	for {
		...
		case p := <-s.addpeer:
			...
			if p.callbackFun != nil {
				if err := p.callbackFun(p, addPeerFlag, len(peers), inboundCount); err != nil && config.IsLogOn(config.NetLog) {
					s.logger.Warnf("[NETWORK] addpeer error: %v", err)
				}
			}
		...
		case p := <-s.delpeer:
			...
			// Peer information change callback
			if p.callbackFun != nil {
				if err := p.callbackFun(p, delPeerFlag, len(peers), inboundCount); err != nil && config.IsLogOn(config.NetLog) {
					s.logger.Warnf("[NETWORK] delpeer error: %v", err)
				}
			}
		}
	}
	...
	for len(peers) > 0 {
		...
		if p.callbackFun != nil {
			if err := p.callbackFun(p, delPeerFlag, len(peers), inboundCount); err != nil && config.IsLogOn(config.NetLog) {
				s.logger.Warnf("[NETWORK] delpeer out loop error: %v", err)
			}
		}
	}
	if config.IsLogOn(config.NetDebugLog) {
...
func (s *Server) checkInboundConn(_ net.Conn, remoteIP net.IP) error {
	if remoteIP == nil {
		return nil
	}
...
func (s *Server) connectPeerChecks(peers map[common.NodeID]*Peer, inboundCount int, p *Peer) error {
	switch {
	case !p.is(trustedConn) && len(peers) >= s.MaxPeersCount:
		return DiscTooManyPeers
	case !p.is(trustedConn) && p.is(inboundConn) && inboundCount >= s.maxInboundConns():
		return DiscTooManyInboundPeers
	case peers[p.ID] != nil:
		return DiscAlreadyConnected
	case p.ID == s.ID:
		return DiscSelf
	case s.DiscoveryType.IsSRT() && p.flag == inboundConn:
...
func SendReasonAndClose(conn net.Conn, enc cipher.Stream, erro error) {
	...
	if r, ok := erro.(DiscReason); ok && r != DiscNetworkError && r != DiscRequested && r != DiscSubprotocolError {
		if err = conn.SetWriteDeadline(time.Now().Add(discTimeout)); err == nil {
			...
			var msgload []byte
			msgload, err = writeMsgload(discMsg, enc)
			if err == nil {
				_, err = conn.Write(msgload)
			}
		}
	}
	if (err != nil || erro != nil) && config.IsLogOn(config.NetDebugLog) {
...
func needPart(payLoadLen int) bool {
	return payLoadLen >= MaxBytesCanBroadcast-c1.RealCipher.LengthOfPublicKey()-c1.RealCipher.LengthOfSignature()
}
...
func (s *Server) BroadcastFullPayLoad(info string, eventType models.EventType,
	...
	var msgLoad []byte
	msgLoad, err = writeMsgload(msg, nil)
	if err != nil {
		return err
	}
	s.Peers.Range(func(key, value interface{}) bool {
...
func (s *Server) BroadcastPartPayLoad(info string, eventType models.EventType,
	...
	msgLoad, err := writeMsgload(msg, nil)
	if err != nil {
		s.logger.Errorf("[NETWORK] (%s) writeMsgload failed: %v", info, err)
		return err
	}
		...
		if v, ok := value.(*Peer); ok {
			if n > 0 {
				...
				if partEventLoad == nil {
					...
					partMsgLoad, err = writeMsgload(partMsg, nil)
					if err != nil {
						partEventLoad = nil
						return true
					}
...
func (s *Server) sendToPeer(info string, toNodes common.NodeIDs, pb interface{}, pub, sig []byte, _ bool) (outpub, outsig []byte, err error) {
	var etype models.EventType
	var eventLoad []byte
	if sig == nil {
		etype, eventLoad, pub, sig, err = WriteEventLoad(pb, true)
	...
	if len(toNodes) == 0 {
		pids := s.PeerIDs()
		r := rand.Intn(len(pids))
		tos = append(tos, pids[r])
	}
	if len(toNodes) > 1 {
		copy(tos[:], toNodes[:])
	...
	_, _, _ = s.RandBroadcast(1, info, pb, pub, sig)

	return pub, sig, nil
}

func (s *Server) sendToChain(_ string, chainId common.ChainID, pb interface{}, pub, sig []byte, _ bool) (outpub, outsig []byte, err error) {
	var etype models.EventType
	var eventLoad []byte
	if sig == nil {
		etype, eventLoad, pub, sig, err = WriteEventLoad(pb, true)
...
func (s *Server) RandBroadcast(size int, info string, msgv interface{}, pub, sig []byte,
	...
	go func() {
		...
		msgLoad, err := writeMsgload(msg, nil)
		if err != nil {
			s.logger.Errorf("[NETWORK] RandBroadcast.writeMsgload ChainID:%d NT:%s H:%x (%s,Len:%d) NID:%s, %s, error: %v",
				s.chainID, s.netType, eventHash[:5], etype, len(eventLoad), s.NodeID(), info, err)
			return
		}
...
func (s *Server) HandleMsg(peer *Peer, msg *Msg) error {
	...
	if valid := models.VerifyMsg(m, pub, sig); !valid {
...
func (s *Server) HandPeerInfo(p *Peer, flag int, peerCount int, inboundCount int) error {
	if config.IsLogOn(config.NetLog) {
		if flag == addPeerFlag {
			s.logger.Infof("[PEER] connected %s flag %s discv %s count %d, inbound count %d", p.Node.String(), p.flag, s.DiscoveryType, peerCount, inboundCount)
		} else if flag == delPeerFlag {
			s.logger.Infof("[PEER] disconnected %s flag %s discv %s count %d, inbound count %d", p.Node.String(), p.flag, s.DiscoveryType, peerCount, inboundCount)
		}
	}
	return nil
}
