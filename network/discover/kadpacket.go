...
func (req *ping) handle(t *udp_kad, from *net.UDPAddr, fromID common.NodeID, mac []byte) error {
	...
	if !config.VersionInfo.ContainsVersion(req.Version) {
		return errVersion
	}
...
func (req *findnode) handle(t *udp_kad, from *net.UDPAddr, fromID common.NodeID, mac []byte) error {
	...
	p := neighbors{Version: uint64(consts.Version), ChainID: t.chainId, NetType: t.netType, Expiration: uint64(time.Now().Add(expiration).Unix())}
	var sent bool
