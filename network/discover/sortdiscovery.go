...
func init() {
	p := neighborsSort{Version: uint64(consts.Version), ChainID: common.NilChainID, NetType: common.BranchDataNet, Expiration: ^uint64(0)}
...
func (t *udp_srt) Version() uint64 {
	return uint64(consts.Version)
}
...
func encodePacketSort(ptype byte, req interface{}) (packet, hash []byte, err error) {
	...
	packet = b.Bytes()
	pub, sig, err := models.SignHash(common.SystemHash256(packet[headSize:]))
	if err != nil {
...
func decodePacketSort(buf []byte) (packetSort, common.NodeID, []byte, error) {
	...
	if !models.VerifyHash(common.SystemHash256(buf[headSize:]), pub, sig) {
		return nil, common.NodeID{}, hash, errors.New("hash signature verify failed")
	}
	pubkey, err := cipher.RealCipher.BytesToPub(pub)
	if err != nil {
		return nil, common.NodeID{}, nil, err
	}
	fromID := common.BytesToNodeID(cipher.RealCipher.PubToBytes(pubkey))
	var req packetSort
