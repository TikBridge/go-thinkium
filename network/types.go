...
func GetEventTypeFromPayLoad(payLoad []byte) (models.EventType, error) {
	l := len(payLoad)
	kl := cipher.RealCipher.LengthOfPublicKey()
	sl := cipher.RealCipher.LengthOfSignature()
...
func PayLoad2Body(payLoad []byte) (eventType models.EventType, body []byte, pub []byte, sig []byte, err error) {
	...
	if bytes.Equal(pub, models.EmptyPublicKey) {
		pub = nil
	}
	sig = payLoad[l-sl:]
	if bytes.Equal(sig, models.EmptySignature) {
		sig = nil
	}
	return
}
...
func UnpackP2PMsg(msg *Msg) (eventLoad []byte, eventType models.EventType, body []byte, pub, sig []byte, err error) {
	if msg.LoadSize() <= models.EventTypeLength+
		cipher.RealCipher.LengthOfPublicKey()+
		cipher.RealCipher.LengthOfSignature() {
...
func PackP2PMsg(eventOrPayLoad, pub, sig []byte) *Msg {
	...
	if len(pub) != cipher.RealCipher.LengthOfPublicKey() {
		payLoad = append(eventOrPayLoad, models.EmptyPublicKey...)
	} else {
		payLoad = append(eventOrPayLoad, pub...)
	}
	if len(sig) != cipher.RealCipher.LengthOfSignature() {
		payLoad = append(payLoad, models.EmptySignature...)
	} else {
