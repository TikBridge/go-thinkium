...
func (s *TcpHandShaker) ShakeHandWith(node *discover.Node) (net.Conn, *Secrets, error) {
	...
	proofload, err := writeMsgload(msg, nil)
	if err != nil {
		return nil, nil, err
	}
...
func (s *TcpHandShaker) VerifyPeerProof(con net.Conn) (*discover.Node, common.ChainID, *Secrets, error) {
	if err := con.SetReadDeadline(time.Now().Add(handshakeTimeout)); err != nil {
...
func sendHandShakeResponse(conn net.Conn) (*HandShakeRsp, error) {
	...
	pubLen := cipher.RealCipher.LengthOfPublicKey()
	sigLen := cipher.RealCipher.LengthOfSignature()
	...
	randomPrivKey, err := cipher.RealCipher.GenerateKey()
	if err != nil {
		return nil, err
	}
	...
	pub, sig, err := models.SignMsg(respData[:hashLen+pubLen])
	copy(respData[hashLen+pubLen:], sig)

	msg := &Msg{MsgType: HandProofMsgType, Payload: respData}
	respload, err := writeMsgload(msg, nil)
	if err != nil {
		return nil, err
	}
	if err := conn.SetWriteDeadline(time.Now().Add(handshakeTimeout)); err != nil {
...
func receiveHandShakeResponse(conn net.Conn, nid common.NodeID) (*HandShakeRsp, error) {
	...
	pub := cipher.RealCipher.PubFromNodeId(nid[:])
	hashLen := SECLen + MACLen
	pubLen := cipher.RealCipher.LengthOfPublicKey()
	sigLen := cipher.RealCipher.LengthOfSignature()

	if respMsg.MsgType != HandProofMsgType {
		if respMsg.MsgType == DiscMsgType {
			r := new(DiscReason)
			_ = rtl.Unmarshal(respMsg.Payload, r)
			return nil, r
		}
	...
	if !models.VerifyMsg(respMsg.Payload[:hashLen+pubLen], pub, sig) {
		return nil, dialErr{errors.New("invalid msg signature received")}
	}
	respNonce := respMsg.Payload[:hashLen]
	respRandPub, err := cipher.RealCipher.BytesToPub(respMsg.Payload[hashLen : hashLen+pubLen])
	if err != nil {
...
func (s *TcpHandShaker) makeProof(remotePub []byte) ([]byte, *HandShakeReq, error) {
	...
	remote, err := cipher.RealCipher.BytesToPub(remotePub)
	if err != nil {
		return nil, nil, err
	}
	shared, err := cipher.SystemPrivKey.GenerateShared(remote, SECLen, MACLen)

	signed := xor(shared, initNonce)

	randPriv, err := cipher.RealCipher.GenerateKey()
	if err != nil {
		return nil, nil, err
	}

	randSig, err := cipher.RealCipher.Sign(randPriv.ToBytes(), signed)
	if err != nil {
		return nil, nil, err
	}
	...
	binary.BigEndian.PutUint64(proof[common.NodeIDBytes+9:common.NodeIDBytes+17], uint64(consts.Version))
	if pmsLen > 0 {
	...
	pub, sig, err := models.SignMsg(proof[:common.NodeIDBytes+17+pmsLen+hasLen+pubLen+sigLen])
	if err != nil {
...
func (s *TcpHandShaker) checkProof(proofload []byte, addr *net.TCPAddr) (*discover.Node, common.ChainID, *HandShakeReq, error) {
	...
	if !models.VerifyMsg(proofload[:common.NodeIDBytes+17+proofLen+hasLen+pubLen+sigLen], pub, sig) {
	...
	if !config.VersionInfo.ContainsVersion(vsn) {
	...
	initPub, err := cipher.RealCipher.BytesToPub(pub)
	if err != nil {
		return nil, common.NilChainID, nil, err
	}

	shared, err := cipher.SystemPrivKey.GenerateShared(initPub, SECLen, MACLen)
	if err != nil {
		return nil, common.NilChainID, nil, err
	}
	...
	if !cipher.RealCipher.Verify(initRandPub, token, initRandSig) {
		s.logger.Errorf("check rand sig failed %d %s %s ", cid, ntt, nid)
		return nil, common.NilChainID, nil, DiscProtocolError
	}

	reqRandPub, err := cipher.RealCipher.BytesToPub(initRandPub)
	if err != nil {
