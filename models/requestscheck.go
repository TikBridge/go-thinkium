...
// 为了避免恢复数据时还要同步恢复0链的chains，链信息由用户输入，在执行时检查本侧数据是否合法即可（因为就算不同步0链
// 数据，本地链信息还是可以知道的）。输入错误可以通过cancel找回
...
func (c *CashCheck) String() string {
	return fmt.Sprintf("Check{Parent:%d Shard:%t From:[%d,%x] Nonce:%d To:[%d,%x]"+
...
// 为了兼容以前的客户端以及历史数据，需要使缺省UseLocal=false时的CashCheck序列化与以前的版本一致，从而保
// 证Hash值一致当开始的ChainID==ReservedMaxChainID时，表示后面会跟对象版本及特殊值
// 版本0x0: 后面会跟一个字节的版本号（0），同时表示UseLocal=true
// 版本0x1，后跟useLocal(1byte), 增加ParentChain(4bytes), IsShard(1byte), CurrencyID(2bytes)
...
func (r *CashRequest) String() string {
	...
	return fmt.Sprintf("CashReq{%s Proofed(ChainID:%d Height:%s Hob:%x) Proof:%s}", r.Check, r.ProofedChainID,
		&(r.ProofHeight), common.ForPrint(r.ProofHeaderHash[:]), r.Proofs)
}
...
func (r *CashRequest) InfoString(level common.IndentLevel) string {
	...
	return fmt.Sprintf("CashReq{"+
		"\n%s\tCheck: %s"+
		"\n%s\tProofed: ChainID:%d Height:%s Hob:%x"+
		"\n%s\tProofs: %s"+
		"\n%s}",
		base, r.Check,
		base, r.ProofedChainID, &(r.ProofHeight), r.ProofHeaderHash[:],
		base, r.Proofs.InfoString(level+1),
		base)
}
...
// 4. ProofedHash==Hash(Block(ChainID:MainChainID, Height:ConfirmedHeight)), the proof target
...
func (c *CancelCashCheckRequest) String() string {
	...
	return fmt.Sprintf("CCCR{%s ChainID:%d Height:%d Header:%x nil(CCCProof):%t nil(Proof):%t}",
		c.Check, c.AbsenceChainID, c.AbsenceHeight, c.ProofedHash[:5], c.CCCProofs == nil, c.Proofs == nil)
}
...
func (c *CancelCashCheckRequest) InfoString(level common.IndentLevel) string {
	...
	base := level.IndentString()
	return fmt.Sprintf("CCCR{"+
		"\n%s\tCheck: %s"+
		"\n%s\tAbsence: ChainID:%d Height:%s"+
		"\n%s\tCCCProofs: %s"+
		"\n%s\tProofs: %s"+
		"\n%s\tProofed: Height:%s Hob:%x"+
		"\n%s}",
		base, c.Check,
		base, c.AbsenceChainID, &(c.AbsenceHeight),
		base, c.CCCProofs.InfoString(level+1),
		base, c.Proofs.InfoString(level+1),
		base, &(c.ConfirmedHeight), c.ProofedHash[:],
		base)
}
...
func (c *CancelCashCheckRequest) Verify(existence bool) (hashOfCheck []byte, hashOfTrie []byte, hashOfProof []byte, err error) {
	...
	if !bytes.Equal(hashOfProof, c.ProofedHash[:]) {
