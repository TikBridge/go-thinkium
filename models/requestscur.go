...
func (c *ExchangerAdminData) Deserialization(r io.Reader) (shouldBeNil bool, err error) {
	...
	if len(rate) > 0 {
		...
		if !ok || rat == nil {
			return false, fmt.Errorf("illegal exchange rate: %s", string(rate))
		}
		c.NewRate = rat
	} else {
		c.NewRate = nil
...
type ExchangerWithdrawData struct {
	RequestAddr common.Address // 提款请求发送地址
	Nonce       uint64         // 提款请求执行的系统合约账户的Nonce，避免重放攻击
	WithdrawTo  common.Address // 提款到的地址
	UseLocal    bool           // 指定提款的货币类型：true-第二币种, false-基础币种
	Value       *big.Int       // 提款数额
}
...
func (d *ExchangerWithdrawData) Deserialization(r io.Reader) (shouldBeNil bool, err error) {
	...
	if len(bs) == 0 {
		...
		if !ok || v == nil {
			return false, fmt.Errorf("restore big.Int failed. (%s)", string(bs))
		}
		d.Value = v
	}
