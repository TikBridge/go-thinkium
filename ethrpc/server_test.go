...
func TestRunEthRpc(t *testing.T) {
	...
	ethrpcsrv, _ := NewServer(conf.NetworkConf.ETHRPC[0].GetRpcEndpoint())
	rpcAPI := []API{
		{
			Namespace: "eth",
			Public:    true,
	...
	if err := RegisterApis(rpcAPI, conf.NetworkConf.ETHRPC[0].GetChainID(), []string{"eth", "web3"}, ethrpcsrv, false); err != nil {
		panic(err)
	}
